...
// ============================================================
// 🎨 THEME ENGINE — visible everywhere
// ============================================================
const THEMES = [
...
(function () {
    ...
    const letterMoods = [
        {
            emoji: "🥺", label: "Open when", title: "you miss me",
        ...
        {
            emoji: "😢", label: "Open when", title: "you're sad",
            signature: "— Always here for you ❤️",
            messages: [
        ...
        {
            emoji: "🌙", label: "Open when", title: "you can't sleep",
            signature: "— Goodnight, my love 🌙",
            messages: [
        ...
        {
            emoji: "😊", label: "Open when", title: "you need a smile",
        ...
        {
            emoji: "😤", label: "Open when", title: "you're angry at me",
            signature: "— I only want to make you smile ❤️",
            messages: [
        ...
        {
            emoji: "🫂", label: "Open when", title: "you feel alone",
            signature: "— You'll never be alone with me ❤️",
            messages: [
        ...
        {
            emoji: "💗", label: "Open when", title: "you want to feel loved",
            signature: "— Forever yours ❤️",
            messages: [
        ...
        {
            emoji: "💪", label: "Open when", title: "you need courage",
            signature: "— I believe in you, always ❤️",
            messages: [
    ...
    function initGameApp() {
        ...
        function saveProgress() {
            localStorage.setItem('loveQuest_progress', JSON.stringify({
                music: musicEnabled, typing: typingSoundEnabled
            }));
        }
        ...
        function typeLetter(text, el) {
            ...
            typingInterval = setInterval(function () {
                if (i < text.length) {
                    ...
                    if (typingSoundEnabled && i % 3 === 0) playTypingTick();
                    i++;
                } else {
        ...
        function playTypingTick() {
            ...
            try {
                ...
                o.start(); o.stop(a.currentTime + 0.025);
            } catch (e) {}
        }
        ...
        function spinWheel() {
            ...
            function frame(now) {
                ...
                if (t < 1) requestAnimationFrame(frame);
                    ...
                    wheelRotation = current % (Math.PI * 2);
        ...
        function finishSpin(winningIndex) {
            ...
            showWheelResult(slice, winningIndex);
            wheelSpinning = false;
            if (spinBtn) spinBtn.disabled = false;
        }

        function showWheelResult(slice, index) {
                ...
                case 'surprise': {
                    ...
                    switch (pick) {
                        case 'kiss':
                            ...
                            if (kissesEl) for (let i = 0; i < 5; i++) {
                                const s = document.createElement('span'); s.textContent = '😘'; kissesEl.appendChild(s);
                            }
                            break;
                        case 'letter':
                            bodyText = LOVE_LETTERS[Math.floor(Math.random() * LOVE_LETTERS.length)];
        ...
        function initExperience() {
            ...
            if (hbHeart) {
                hbHeart.addEventListener('click', function () {
            ...
            if (starA) starA.addEventListener('click', function () {
                universeA = true;
                if (msgA) msgA.classList.add('show');
                checkUniverse();
            });
            if (starB) starB.addEventListener('click', function () {
                universeB = true;
                if (msgB) msgB.classList.add('show');
                checkUniverse();
            });
            ...
            if (openAnotherGiftBtn) {
                openAnotherGiftBtn.addEventListener('click', function () {
            ...
            const catchFinal = document.getElementById('catchFinal');
            const restartCatchBtn = document.getElementById('restartCatchBtn');
            let catchScore = 0;
            let catchTimer = null;
            const CATCH_TARGET = 15;
            const CATCH_MESSAGES = [
                '❤️ +1 — You are beautiful', '❤️ +1 — You are loved',
                '❤️ +1 — You are my favorite', '❤️ +1 — You are precious',
                '❤️ +1 — You are enough', '❤️ +1 — You are my home',
                '❤️ +1 — You are my everything'
            ];
            ...
            function spawnCatchHeart() {
                ...
                h.style.left = (20 + Math.random() * (areaW - 60)) + 'px';
                h.style.animationDuration = (3 + Math.random() * 2) + 's';
                h.addEventListener('click', function () {
                    catchScore++;
                    if (catchCount) catchCount.textContent = catchScore;
                    const m = CATCH_MESSAGES[Math.floor(Math.random() * CATCH_MESSAGES.length)];
                    if (catchMsg) catchMsg.textContent = m;
                    burstHearts(h, 6);
                    h.remove();
                    if (catchScore >= CATCH_TARGET) {
                        if (catchFinal) catchFinal.classList.remove('hidden');
                        if (catchTimer) clearInterval(catchTimer);
                        markVisit('catch');
                    }
                });
                catchArea.appendChild(h);
                setTimeout(function () { if (h.parentNode) h.remove(); }, 5200);
            }
            ...
            function startCatch() {
                ...
                if (catchMsg) catchMsg.textContent = 'Tap the hearts ❤️';
                if (catchFinal) catchFinal.classList.add('hidden');
                if (catchTimer) clearInterval(catchTimer);
                catchTimer = setInterval(spawnCatchHeart, 700);
            }
            if (restartCatchBtn) restartCatchBtn.addEventListener('click', startCatch);
            startCatch();
            ...
            if (closeCandleLetter) {
                closeCandleLetter.addEventListener('click', function () {
            ...
            const queenMessage = document.getElementById('queenMessage');
            if (queenEnterBtn) {
                queenEnterBtn.addEventListener('click', function () {
                    if (queenMessage) queenMessage.classList.remove('hidden');
                    burstHearts(queenEnterBtn, 20);
                    markVisit('queen');
                });
            }

            // 🌷 GARDEN — grows as sections are visited
            updateGarden();
